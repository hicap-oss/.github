...
A reply confirms the Hicap backend is connected and routing correctly. To confirm the endpoint independently of the SDK:
...
| `401` invalid subscription key | Env var not loaded | In Next.js, keep the provider in server-only code so `process.env.HICAP_API_KEY` resolves |
